...
task check(input [8*8:1] name, input [31:0] actual, input [31:0] expected);
	begin
		if (actual !== expected) begin
			$display("FAIL %0s: got %0d (0x%h), expected %0d (0x%h)", name, actual, actual, expected, expected);
			errors = errors + 1;
		end else begin
...
initial begin
	...
	$display("-----------------------------------------------");
	check("x1",     uut.registers.registers[1],  32'd5);
	check("x2",     uut.registers.registers[2],  32'd10);
	check("x3",     uut.registers.registers[3],  32'd15);
	check("x4",     uut.registers.registers[4],  32'd0);   // must stay 0: skipped by beq
	check("x5",     uut.registers.registers[5],  32'd30);
	check("x10",    uut.registers.registers[10], 32'd5);
	check("x11",    uut.registers.registers[11], 32'd3);
	check("x12",    uut.registers.registers[12], 32'd13);
	check("x13",    uut.registers.registers[13], 32'd1);
	check("x14",    uut.registers.registers[14], 32'd8);
	check("x15",    uut.registers.registers[15], 32'd2);
	check("mem[8]", uut.data_Memory.D_memory[2], 32'd15);  // sw x3, 8(x0) -> word index 2
	$display("-----------------------------------------------");
	if (errors == 0)
